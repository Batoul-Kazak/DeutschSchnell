...
//         queryFn: async () => {
//             const res = await fetch(`/data/tests/${level}.json`);
...
export default function Tests({level: propLevel}) {
    ...
    const {
        ...
        selectedAnswerId,
        questions,
    } = useTestLogic({ levelData: levelData, level: level || '' });
    ...
    if (isLoading) {
        return <LoadingTest level={level} />;
    }

    if (error || !levelData) {
        return (
            <NotFound
                message="Sorry the requested test is unavailable for now, we are working on adding more tests!"
                link="tests"
                buttonText="Go Back to Tests Page"
            />
        );
    }
