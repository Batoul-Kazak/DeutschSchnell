...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    const filtered =
      selectedLevel === "All"
        ? users
        : users.filter((u) => u.level === selectedLevel);

    const genderCount = filtered.reduce(
      (acc, curr) => {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-700">
            <h3 className="text-xl font-bold text-light-yellow mb-6">
              Age Distribution
            </h3>
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        <div className="w-16 bg-gray-600 rounded-full h-1.5">
                          <div
                            className="bg-light-green h-1.5 rounded-full"
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-bold ${
                          user.status === "Active"
                            ? "bg-dark-green/20 text-light-green border border-dark-green"
                            : user.status === "At Risk"
                              ? "bg-dark-red/20 text-light-red border border-dark-red"
